...
#  - Решение: выполнить идемпотентный DDL через отдельную команду
#    (ADD COLUMN IF NOT EXISTS работает в PostgreSQL 9.6+), потом
#    stamp — Alembic считает, что миграция применена, и в следующий
#    раз не накатывает.
#
...
#
# При добавлении новой миграции с новой колонкой: (1) создать файл с
# down_revision = 's1y2s3t4u5m6'; (2) добавить идемпотентный DDL в
# команду ensure-system-sku (или создать новую ensure-<feature>);
