...
CMD ["sh", "-c", "if [ \"$RESET_DB\" = \"1\" ]; then FLASK_APP=wsgi.py flask reset-public-schema --yes; fi && FLASK_APP=wsgi.py flask db-init && FLASK_APP=wsgi.py flask fix-password-length && FLASK_APP=wsgi.py flask db stamp heads && if [ -n \"$CREATE_ADMIN_LOGIN\" ] && [ -n \"$CREATE_ADMIN_PASSWORD\" ] && [ -n \"$CREATE_ADMIN_EMAIL\" ]; then FLASK_APP=wsgi.py flask create-admin --login \"$CREATE_ADMIN_LOGIN\" --password \"$CREATE_ADMIN_PASSWORD\" --email \"$CREATE_ADMIN_EMAIL\"; fi && gunicorn wsgi:app --bind 0.0.0.0:${PORT:-5000} --workers ${GUNICORN_WORKERS:-2} --timeout 60 --access-logfile - --error-logfile -"]
